...
def run_simul(version_idx_str):
    ...
    with open(f'C:\\script9\\ML\\SIMUL_{version_idx_str}\\run_bat_{version_idx_str}.bat',"w") as f :
        f.write(f'"C:\\Program Files\\AnsysEM\\AnsysEM21.1\\Win64\\ansysedt.exe" -iconic -runscript "C:\\script9\\ML\\SIMUL_{version_idx_str}\\run_ansys_{version_idx_str}.py"')
...
for i in range(1, 10000): 
    ...
    try :
        try:
            ...
            time.sleep(1)
        shutil.copy(f'C:\script9\ML_aedt\ML_ref.aedt',f'C:\script9\ML_aedt\ML9.aedt')
        time.sleep(1)
        run_simul(i)
        time.sleep(1)
        if os.path.isfile(f'C:\script9\ML_aedt\ML9.aedt') :
            os.remove(f'C:\script9\ML_aedt\ML9.aedt')
        time.sleep(1)	
        rmdir(f'C:\script9\ML_aedt\ML9.aedtresults')
        try:
            os.remove(f'C:\script9\ML_aedt\ML9.aedt.lock')
        except:
            time.sleep(1)
