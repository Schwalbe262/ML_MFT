...
REFERENCE_SCRIPT_FILE_NAME = f'C:\\script1\\run_ansys_ref.py'
...
def run_simul(version_idx_str):
    ...
    os.mkdir(f'C:\script1\ML\SIMUL_{version_idx_str}')

    #2 Make Variable info file
    with open(f'C:\script1\ML\SIMUL_{version_idx_str}\info.yaml', "w") as info_file:
    ...
    #Save file
    with open(f'C:\\script1\\ML\\SIMUL_{version_idx_str}\\run_ansys_{version_idx_str}.py',"w") as f :
    ...
    with open(f'C:\\script1\\ML\\SIMUL_{version_idx_str}\\run_bat_{version_idx_str}.bat',"w") as f :
        f.write(f'"C:\\Program Files\\AnsysEM\\AnsysEM21.1\\Win64\\ansysedt.exe" -iconic -runscriptandexit "C:\\script1\\ML\\SIMUL_{version_idx_str}\\run_ansys_{version_idx_str}.py"')

    workingDir = f'C:\script1\ML\SIMUL_{version_idx_str}'
    executeFile = f'C:\script1\ML\SIMUL_{version_idx_str}\\run_bat_{version_idx_str}.bat'
    os.chdir(workingDir)
...
for i in range(1, 10000): 
    ...
    try :
        try:
            os.remove(f'C:\script1\ML_aedt\ML1.aedt.lock')
        except:
            time.sleep(1)
        if os.path.isfile(f'C:\script1\ML_aedt\ML1.aedt') :
            os.remove(f'C:\script1\ML_aedt\ML1.aedt')
        time.sleep(1)	

        shutil.copy(f'C:\script1\ML_aedt\ML_ref.aedt',f'C:\script1\ML_aedt\ML1.aedt')
        time.sleep(1)
        ...
        rmdir(f'C:\script1\ML_aedt\ML1.aedtresults')
        try:
            os.remove(f'C:\script1\ML_aedt\ML1.aedt.lock')
        except:
            time.sleep(1)
