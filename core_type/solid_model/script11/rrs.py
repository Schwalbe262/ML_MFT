...
def run_simul(version_idx_str):
    ...
    space1_range = [4, 50, 2, 0] 
    space2_range = [4, 50, 2, 0] 
    space3_range = [4, 50, 2, 0] 
    space4_range = [4, 50, 2, 0] 
    ...
    airx = 1.2 * (w1 + 2*(max(2*space1+coil_width1,2*space3+coil_width2)))
    ...
    parameter = np.array([N1,w1,l1,l2,h1,space1,space2,space3,space4,coil_width1,coil_width2,move_z1,move_z2,offset_z1,offset_z2])
    ...
    data1 = np.loadtxt(f'Z:\Autosimul_data\MFT\core_type_solid\{COMPUTER_NAME}\script11\magnetizing_inductance.csv', delimiter=",")
    new_data1 = np.vstack((data1, temp1))
    np.savetxt(f'Z:\Autosimul_data\MFT\core_type_solid\{COMPUTER_NAME}\script11\magnetizing_inductance.csv',new_data1,delimiter=",")

    data2 = np.loadtxt(f'Z:\Autosimul_data\MFT\core_type_solid\{COMPUTER_NAME}\script11\leakage_inductance.csv', delimiter=",")
    new_data2 = np.vstack((data2, temp2))
    np.savetxt(f'Z:\Autosimul_data\MFT\core_type_solid\{COMPUTER_NAME}\script11\leakage_inductance.csv',new_data2,delimiter=",")

    data3 = np.loadtxt(f'Z:\Autosimul_data\MFT\core_type_solid\{COMPUTER_NAME}\script11\loss.csv', delimiter=",")
    new_data3 = np.vstack((data3, temp3))
    np.savetxt(f'Z:\Autosimul_data\MFT\core_type_solid\{COMPUTER_NAME}\script11\loss.csv',new_data3,delimiter=",")
